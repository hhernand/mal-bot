    ...
    if (data.length == 3) {
      ...
      access.tagByName(name, con, function(res) {
        if (res.length == 1) {
          ...
          con.query(sql);
          msg.channel.send(msg.author + ' Tag has been created.');
        }
      })
    }
  },
      ...
      access.tagByName(tname, con, function(res) {
        if (res.length == 1) {
          if (res[0].memberID == id) {
            ...
            con.query(sql);
            msg.channel.send(msg.author + ' Tag has been updated.');
          }
          else {
            msg.channel.send(msg.author + ' You cannot edit that tag.');
          }
        }
        else {
          msg.channel.send(msg.author + ' Tag is not in the database.');
        }
      });
    }
  },
